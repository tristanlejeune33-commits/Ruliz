...
| **5 — Traduction de l'interface du panel** | `src/components/shared/panel-lang-context.tsx` | Le `PanelLangProvider` exposait `lang: initialLang` **sans état React local** : au clic sur une langue, il écrivait le cookie + `router.refresh()` et attendait que le serveur repropage la valeur. Ce round-trip ne mettait pas à jour le contexte client → **le drapeau ne changeait jamais** et l'`useEffect([lang])` de l'`AutoTranslateWrapper` ne se redéclenchait jamais → interface jamais retraduite. | État local `useState(initialLang)` mis à jour **immédiatement** au clic (drapeau + auto-traduction instantanés), + cookie + `router.refresh()` pour synchroniser la sidebar serveur. Le système de traduction (clé Anthropic) était OK ; c'était un bug d'état. | ✅ |
