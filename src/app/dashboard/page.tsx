...
import type { DashboardLayout } from "@/server/dashboard/dashboard-layout-actions";
...
/** Coerce le JSON Prisma en DashboardLayout sûr (ou null si forme invalide). */
function parseDashboardLayout(raw: unknown): DashboardLayout | null {
  ...
  return {
    order: o.order.filter((v): v is string => typeof v === "string"),
    hidden: o.hidden.filter((v): v is string => typeof v === "string"),
  };
}
...
export default async function DashboardHome() {
  ...
  const dashboardLayout = parseDashboardLayout(userPref?.dashboardLayout);
  ...
  // Sections réordonnables/masquables de l'accueil (cf. DashboardCustomizer).
  // Le WelcomeHero reste épinglé en tête, hors customizer.
          ...
          <CardHeader className="relative flex flex-col items-start gap-4 space-y-0 md:flex-row md:items-start md:justify-between">
            <div>
              ...
              <CardDescription className="mt-1">
                Glisse-dépose tes catégories, modifie tes plats, aperçu en
                direct à droite. Tes modifications sont sauvegardées toutes
                seules.
              </CardDescription>
            </div>
